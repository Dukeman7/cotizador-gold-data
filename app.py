...
st.set_page_config(page_title="Cotizador Maestro GD", page_icon="🦅")

# --- MOTOR DE CÁLCULO (LA TRÍADA) ---
def calcular_precio(bw, a, n, c):
    return (a / (bw**n)) + c

# --- PARÁMETROS CALIBRADOS (Team GIAN & LUIS - V11) ---
params = {
...
st.markdown("### Selección de Plan y Cálculo de Rentabilidad")

# Sidebar
st.sidebar.header("Parámetros de Venta")
servicio = st.sidebar.selectbox("Tipo de Servicio / Tecnología", list(params.keys()))
mbps = st.sidebar.number_input("Ancho de Banda (Mbps)", min_value=1.0, value=100.0, step=10.0)
...
col2.metric("Facturación Estimada", f"${total_mensual:,.2f} USD")

# Banda de Seguridad para Ilse
st.info(f"**Rango de Negociación:** Suelo: ${promedio*0.9:.2f} / Techo: ${promedio*1.1:.2f}")
